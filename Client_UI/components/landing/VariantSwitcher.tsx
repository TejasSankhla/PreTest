    ...
    <div className="fixed bottom-6 right-6 z-50">
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button className="flex items-center gap-2 px-4 py-2.5 bg-white/90 backdrop-blur-xl rounded-full shadow-lg border border-gray-200 hover:shadow-xl transition-all text-sm font-medium text-gray-700 hover:text-gray-900 group">
        ...
        <DropdownMenuContent
          align="end"
          className="w-64 bg-white rounded-xl shadow-xl border border-gray-100 p-2"
        >
          {variants.map((v) => (
            <DropdownMenuItem
              key={v.id}
              ...
              <div className="flex flex-col">
                <span
                  className={`font-semibold text-sm ${
                    variant === v.id ? "text-secondary" : "text-gray-900"
                  }`}
                >
                  {v.name}
                </span>
